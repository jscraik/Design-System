...
AStudioComponents provides reusable SwiftUI primitives that mirror React component APIs, enabling developers to build native iOS, macOS, and visionOS applications with familiar patterns and consistent behavior. All components use AStudioFoundation tokens exclusively and support automatic light/dark mode.
...
Add AStudioComponents as a dependency in your `Package.swift`:
...
.target(
    name: "YourTarget",
    dependencies: ["AStudioComponents", "AStudioFoundation", "AStudioThemes"]
)
```
...
- Build the package: `cd platforms/apple/swift/AStudioComponents && swift build`
...
AStudioComponents follows these principles:

1. **Composition Over Inheritance**: Components compose primitives rather than subclass
2. **Token-Driven**: All styling uses AStudioFoundation tokens exclusively
...
- **3.4**: Exclusive use of AStudioFoundation tokens with compile-time safety
