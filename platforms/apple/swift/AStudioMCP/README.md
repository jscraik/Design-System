...
AStudioMCP provides a Swift networking layer to communicate with existing web-based MCP infrastructure, widget rendering using native SwiftUI components styled with AStudioFoundation tokens, and macOS-specific authentication flows using Keychain.
...
Add AStudioMCP as a dependency in your `Package.swift`:

```swift
dependencies: [
    .package(path: "../AStudioMCP")
]
```
...
- Uses `AStudioComponents` primitives for UI elements
- Supports light/dark mode automatically
- Provides platform-appropriate interactions

## MCP Tool Contracts

AStudioMCP is compatible with all existing MCP tool contracts defined in `platforms/mcp/tool-contracts.json`:
...
AStudioMCP provides comprehensive error handling through the `MCPError` enum:
...
AStudioMCP includes comprehensive unit tests:
...
- AStudioFoundation: Semantic design tokens
- AStudioComponents: Reusable SwiftUI primitives
