...
AStudioShellChatGPT provides ready-to-use shell components for building complete native macOS applications with ChatGPT-style visual design. This package is completely optional and can be used independently or alongside the core aStudio packages.
...
- **AStudioFoundation**: Semantic tokens and platform utilities
- **AStudioComponents**: Reusable SwiftUI primitives
- **AStudioThemes**: ChatGPT-style visual constants
...
2. Use individual components (for example, just `VisualEffectView`)
3. Build custom layouts using only AStudioFoundation and AStudioComponents
...
- **5.5**: Seamless integration with AStudioFoundation and AStudioComponents
...
```bash
cd platforms/apple/swift/AStudioShellChatGPT
swift build
```
...
- Build the package: `cd platforms/apple/swift/AStudioShellChatGPT && swift build`
