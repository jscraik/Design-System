...
final class SpotlightManagerSecurityTests: XCTestCase {
    ...
    func testQueryWithControlCharacters() async throws {
        ...
        for query in controlCharQueries {
            ...
            } catch SpotlightError.invalidContent {
    ...
    func testQueryLengthValidation() async throws {
        ...
        XCTAssertThrowsError(
            try await spotlightManager.searchChatMessages(query: invalidLengthQuery, limit: 10),
            "Query exceeding 100 characters should be rejected"
        ) { error in
            XCTAssertTrue(
                error is SpotlightError,
                "Should throw SpotlightError for invalid length"
            )
            if case SpotlightError.invalidContent = error {
                // Expected
            } else {
        ...
        XCTAssertThrowsError(
            try await spotlightManager.searchChatMessages(query: veryLongQuery, limit: 10)
        ) { error in
            XCTAssertTrue(error is SpotlightError)
        }
    }
    ...
    func testQuerySanitizationTableDriven() async throws {
        ...
        let testCases: [SanitizationTestCase] = [
            // Valid queries
            ("simple search", true, "Simple alphanumeric query"),
            ("search with spaces", true, "Query with spaces"),
            ("test-query_123", true, "Query with dashes and underscores"),
            ("email@test.com", true, "Query with @ and ."),
            ("user@domain.com", true, "Email-like query"),
            ("what? really?", true, "Query with question marks"),
            ("wow!", true, "Query with exclamation"),
            ("test.file", true, "Query with dots"),
            ("$100", true, "Query with dollar sign"),
            ("50% off", true, "Query with percent"),
            ("test@example.com", true, "Email address"),

            // Malicious queries that should be sanitized
            ("'; DROP TABLE; --", true, "SQL injection (sanitized)"),
            ("<script>alert(1)</script>", true, "XSS attempt (sanitized)"),
            ("../../etc/passwd", true, "Path traversal (sanitized)"),
            ("$(whoami)", true, "Command injection (sanitized)"),
            ("test\x00null", true, "Null byte (sanitized)"),

            // Invalid queries
            (String(repeating: "a", count: 101), false, "Too long (>100 chars)"),
            ("", true, "Empty (allowed or rejected)"),
            ("   ", true, "Whitespace only (allowed or rejected)")
        ]
    ...
    func testSearchPerformanceWithMaliciousInput() async throws {
        ...
        measure {
            ...
            _ = await withTaskGroup(of: Void.self) { group in
                group.addTaskUnlessCancelled { await group.value }
                group.addTaskUnlessCancelled { await timeoutTask.value }
            }
        }
    }
