...
final class CryptoManagerTests: XCTestCase {
    ...
    func testEncryptAndDecryptData() throws {
        ...
        let encryptedData = try cryptoManager.encrypt(originalData)
        let decryptedData = try cryptoManager.decrypt(encryptedData)
    ...
    func testEncryptAndDecryptString() throws {
        ...
        let encryptedData = try cryptoManager.encryptString(originalString)
        let decryptedString = try cryptoManager.decryptString(encryptedData)
    ...
    func testMultipleEncryptionsOfSameData() throws {
        let originalData = "Consistent data".data(using: .utf8)!

        let encrypted1 = try cryptoManager.encrypt(originalData)
        let encrypted2 = try cryptoManager.encrypt(originalData)
        ...
        let decrypted1 = try cryptoManager.decrypt(encrypted1)
        let decrypted2 = try cryptoManager.decrypt(encrypted2)
    ...
    func testEncryptEmptyData() throws {
        ...
        let encryptedData = try cryptoManager.encrypt(emptyData)
        let decryptedData = try cryptoManager.decrypt(encryptedData)
    ...
    func testEncryptLargeData() throws {
        ...
        let encryptedData = try cryptoManager.encrypt(largeData)
        let decryptedData = try cryptoManager.decrypt(encryptedData)
    ...
    func testEncryptSmallData() throws {
        let smallData = Data([0x01, 0x02, 0x03])

        let encryptedData = try cryptoManager.encrypt(smallData)
        let decryptedData = try cryptoManager.decrypt(encryptedData)
    ...
    func testEncryptVariableSizes() throws {
    ...
    func testDecryptInvalidData() {
        ...
        XCTAssertThrowsError(try cryptoManager.decrypt(invalidData)) { error in
            XCTAssertTrue(
    ...
    func testDecryptEmptyData() {
        ...
        XCTAssertThrowsError(try cryptoManager.decrypt(emptyData)) { error in
    ...
    func testDecryptTooShortData() {
        ...
        XCTAssertThrowsError(try cryptoManager.decrypt(tooShortData)) { error in
            XCTAssertTrue(error is CryptoManager.CryptoError)
        }
    }

    func testDecryptModifiedCiphertext() throws {
        let originalData = "Test data".data(using: .utf8)!
        let encryptedData = try cryptoManager.encrypt(originalData)
        ...
        XCTAssertThrowsError(try cryptoManager.decrypt(modifiedData)) { error in
            XCTAssertTrue(
                error is CryptoManager.CryptoError,
    ...
    func testDecryptWithWrongKey() throws {
        ...
        let encrypted = try cryptoManager.encrypt(originalData)

        // Create a new manager with a different key
        let differentManager = CryptoManager()

        // Try to decrypt with wrong key
        XCTAssertThrowsError(try differentManager.decrypt(encrypted)) { error in
            XCTAssertTrue(
                error is CryptoManager.CryptoError,
    ...
    func testImportInvalidKeySize() {
        let invalidKeySizes = [0, 16, 24, 31, 33, 64]

        for keySize in invalidKeySizes {
            let invalidKeyData = Data(repeating: 0x00, count: keySize)

            if keySize == 32 {
                // Should succeed
                _ = CryptoManager(keyData: invalidKeyData)
            } else {
                // Should fail with precondition failure
                XCTAssertFalse(
                    { () -> Bool in
                        do {
                            _ = CryptoManager(keyData: invalidKeyData)
                    ...
                    "Key size \(keySize) should be rejected"
                )
            }
        }
    }

    func testDifferentManagersHaveDifferentKeys() throws {
        let manager1 = CryptoManager()
        let manager2 = CryptoManager()

        let key1 = manager1.exportKey()
        let key2 = manager2.exportKey()
        ...
        let encrypted1 = try manager1.encrypt(testData)
        let encrypted2 = try manager2.encrypt(testData)
        ...
        XCTAssertThrowsError(try manager2.decrypt(encrypted1)) { error in
            XCTAssertTrue(error is CryptoManager.CryptoError)
        }
    }
    ...
    func testEncryptStringWithSpecialCharacters() throws {
        let specialStrings = [
            "Hello, World!",
            "🔐🔑 Secure! 💪",
            "特殊字符测试",
        ...
        for string in specialStrings {
            let encrypted = try cryptoManager.encryptString(string)
            let decrypted = try cryptoManager.decryptString(encrypted)
    ...
    func testEncryptStringWithNewlinesAndTabs() throws {
        let multilineString = """
        Line 1
        Line 2\tWith Tab
        Line 3
        ...
        let encrypted = try cryptoManager.encryptString(multilineString)
        let decrypted = try cryptoManager.decryptString(encrypted)

        XCTAssertEqual(decrypted, multilineString, "Multiline string should round-trip")
    }

    func testDecryptInvalidUTF8Data() throws {
        // Create valid data, encrypt it
        let validString = "Valid UTF-8"
        let encrypted = try cryptoManager.encryptString(validString)
        ...
        let encryptedNonUTF8 = try cryptoManager.encrypt(nonUTF8Data)

        XCTAssertThrowsError(
            try cryptoManager.decryptString(encryptedNonUTF8)
        ) { error in
            XCTAssertTrue(
                error is CryptoManager.CryptoError,
    ...
    func testErrorDescriptions() throws {
        ...
        let testCases: [ErrorTestCase] = [
            {
                let invalidData = Data([0x00, 0x01])
                return ErrorTestCase(
                    operation: { try self.cryptoManager.decrypt(invalidData) },
...

            {
                ...
                let encrypted = try! self.cryptoManager.encrypt(validData)
                var modified = encrypted
                modified[0] = modified[0] ^ 0xFF
                return ErrorTestCase(
                    operation: { try self.cryptoManager.decrypt(modified) },
                    expectedError: .decryptionFailed(CryptoKit.Error(any: "")),
        ...
        for testCase in testCases {
            XCTAssertThrowsError(
                ...
                if let cryptoError = error as? CryptoManager.CryptoError {
                    ...
                    if case testCase.expectedError = cryptoError {
                        // Match
                    }
                }
            }
    ...
    func testEncryptionDecryptionTableDriven() throws {
        ...
        let testCases: [EncryptionTestCase] = [
            (
                "Empty data",
                Data(),
                "Empty data should encrypt/decrypt"
            ),
            (
                "Single byte",
                Data([0x42]),
                "Single byte should encrypt/decrypt"
            ),
            (
                "Short text",
                "Hello".data(using: .utf8)!,
                "Short text should encrypt/decrypt"
            ),
            (
                "AES block size",
            ...
            (
                "Random data",
                Data((0..<100).map { _ in UInt8.random(in: 0...255) }),
                "Random bytes should encrypt/decrypt"
            ),
            (
                "Unicode text",
                "Hello 世界 🌍".data(using: .utf8)!,
                "Unicode should encrypt/decrypt"
            ),
            (
                "Large data",
                Data(repeating: 0xAB, count: 10_000),
                "Large data should encrypt/decrypt"
            ),
            (
                "All zeros",
                Data(repeating: 0x00, count: 256),
                "All zeros should encrypt/decrypt"
            ),
            (
                "All ones",
                Data(repeating: 0xFF, count: 256),
                "All ones should encrypt/decrypt"
            ),
            (
                "Repeated pattern",
                Data(repeating: 0xAA, count: 512),
                "Repeated pattern should encrypt/decrypt"
            )
        ]

        for testCase in testCases {
            let encrypted = try cryptoManager.encrypt(testCase.data)
            let decrypted = try cryptoManager.decrypt(encrypted)
    ...
    func testSmallDataPerformance() throws {
    ...
    func testConcurrentEncryptionDecryption() async throws {
        ...
        let encryptedData = try cryptoManager.encrypt(testData)

        await withTaskGroup(of: Data.self) { group in
            // Launch multiple concurrent encryption tasks
            for _ in 0..<10 {
                group.addTask {
                    return try! self.cryptoManager.encrypt(testData)
                }
            }
        ...
        await withTaskGroup(of: Data.self) { group in
            for _ in 0..<10 {
                group.addTask {
                    return try! self.cryptoManager.decrypt(encryptedData)
                }
            }
    ...
    func testActorSerialization() async throws {
        ...
        async let enc1: Data = cryptoManager.encrypt(data1)
        async let enc2: Data = cryptoManager.encrypt(data2)
        async let enc3: Data = cryptoManager.encrypt(data3)
        ...
        async let dec1: Data = cryptoManager.decrypt(e1)
        async let dec2: Data = cryptoManager.decrypt(e2)
        async let dec3: Data = cryptoManager.decrypt(e3)
    ...
    func testEncryptionProvidesConfidentiality() throws {
        let sensitiveData = "Password: SuperSecret123!".data(using: .utf8)!

        let encrypted = try cryptoManager.encrypt(sensitiveData)
    ...
    func testEncryptionProvidesIntegrity() throws {
        ...
        XCTAssertThrowsError(try cryptoManager.decrypt(tamperedData)) { error in
            XCTAssertTrue(
                error is CryptoManager.CryptoError,
    ...
    func testDifferentKeysProduceDifferentCiphertexts() throws {
        ...
        let enc1 = try manager1.encrypt(data)
        let enc2 = try manager2.encrypt(data)
        let enc3 = try manager3.encrypt(data)
    ...
    func testEncryptDecryptWithZeroData() throws {
        let zeroData = Data(repeating: 0x00, count: 1024)

        let encrypted = try cryptoManager.encrypt(zeroData)
        let decrypted = try cryptoManager.decrypt(encrypted)
    ...
    func testEncryptDecryptWithPatternedData() throws {
        let patternedData = Data((0..<256).map { $0 })

        let encrypted = try cryptoManager.encrypt(patternedData)
        let decrypted = try cryptoManager.decrypt(encrypted)
