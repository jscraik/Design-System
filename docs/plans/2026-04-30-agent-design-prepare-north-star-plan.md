...
- Validate every returned `packageScript`, when present, against the root `package.json` script map.
...
- Returned validation commands are all read-only and reference existing package scripts when a `packageScript` is present.
...
| AC5 | Returned validation commands are read-only by default, and any `packageScript` maps to an existing root package script. | SA4, SA5 |
...
| Follow-up: Agent-First Prepare Evidence Gate | completed | Codex | `pnpm agent-design:prepare:changed` now builds the prepare stack, discovers changed protected `.tsx`/`.jsx` UI surfaces, runs direct read-only CLI prepare for each surface, and fails closed on unsafe, incomplete, unparseable, missing, or outside-repo payloads. README, workflow docs, PR template, GitHub Actions web CI, and `FORJAMIE.md` point agents at this gate before PR handoff. |
