...
- **2026-03-04:** Hardened `.github/workflows/release-guidance.yml` to run package-local installs/checks for `@brainwav/design-system-guidance` only (`pnpm -C packages/design-system-guidance install --ignore-workspace --lockfile=false`), package-local validation commands, and pnpm-compatible publish args (removed unsupported `--no-git-checks`). This avoids unrelated root/workspace dependency fetch failures and dirty-tree publish failures during release runs. Impact: guidance release workflow can validate/publish independently and reliably.
