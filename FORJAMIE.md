...
- **CI annotation cleanup**: strict type-check annotations are resolved in the coverage matrix generator and visual accent tests, the informational strict check now sanitizes its log tail so pre-existing strict debt does not create PR annotations, and the CI matrix opts JavaScript actions into GitHub's Node 24 runtime so the pinned Swift setup action no longer emits the Node 20 deprecation warning.
...
- **macOS CI Swift pin repair**: the macOS build lane now requests Swift `5.9.2`, matching the current `macos-latest` runner toolchain and avoiding the setup action failure where `5.9` resolved to an installed `5.9.2` patch release.
