...
- **2026-03-05:** Re-enabled the `risk-policy-gate` job in `.github/workflows/pr-pipeline.yml` (removed unconditional `if: ${{ false }}` skip) and corrected its docs baseline step to `node scripts/check-doc-links.mjs` (the prior script names either did not exist or required `vale` binary not present in that runner). Impact: PR policy and quality gates are enforceable rather than globally bypassed, and the risk gate no longer fails before policy evaluation on missing tooling.
