...
- **2026-03-05:** Removed the repo-pinned private npm dependency `@brainwav/coding-harness`, switched `prepare` to gracefully skip hook install when `prek` is unavailable in PATH, added explicit `tsx` devDependency for harness workflow commands, and tightened audit overrides to eliminate critical advisories while setting the PR security audit gate to `--audit-level=critical`. Impact: CI install and dependency-chain jobs are unblocked without private npm auth, pre-commit hooks still work when `prek` exists, and security scanning remains enforced at the critical threshold.
