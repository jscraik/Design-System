...
- **2026-03-05:** Re-enabled the `risk-policy-gate` job in `.github/workflows/pr-pipeline.yml` (removed unconditional `if: ${{ false }}` skip) and corrected its docs baseline command to `pnpm doc:lint` (instead of missing `docs:lint`) so downstream `review-gate`, `evidence-verify`, `remediation-decision`, and quality checks execute again. Impact: PR policy and quality gates are enforceable rather than globally bypassed, and the gate no longer fails on a missing script name.
