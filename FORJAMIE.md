...
- **2026-02-25:** Restored Apps SDK compatibility exports at the `@design-studio/ui` root by aliasing integrations exports (`AppsSDKButton`, `AppsSDKBadge`, `AppsSDKInput`, `AppsSDKUIProvider`, etc.) in `packages/ui/src/index.ts`. This unblocks web harness/template imports that expect `AppsSDK*` names. Impact: keyboard-navigation Playwright suite at `platforms/web/apps/web/tests/keyboard-navigation.spec.ts` is green again (20/20 passing) instead of timing out on missing exports.
