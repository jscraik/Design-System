...
- **2026-03-04:** Added a fast-fail npm auth gate to `.github/workflows/release-guidance.yml` (`npm whoami` using `BRAINWAV_NPM_TOKEN`) before dependency install/publish in the publish job. Impact: token/scope problems now fail early with explicit auth diagnostics instead of failing later during `npm publish`.
