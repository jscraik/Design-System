...
- **2026-03-05:** Synced the Storybook 10.2.10 bump across workspace package manifests and root pnpm policy in `package.json`, `packages/effects/package.json`, `packages/ui/package.json`, `platforms/web/apps/storybook/package.json`, `platforms/web/apps/storybook/vitest.config.ts`, and `pnpm-lock.yaml`. Follow-up test config cleanup added `@storybook/addon-vitest` to `packages/ui` (where the stories live) and made Storybook Vitest run in browser mode by default, matching the addon’s current template expectations. Impact: the Dependabot Storybook PR now keeps core, adapters, addons, and the root override/peer-allowlist aligned so frozen installs and CI use one coherent Storybook version without failing the Storybook test harness on package-resolution or browser-mode drift.
