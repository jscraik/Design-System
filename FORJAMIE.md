...
- **2026-03-04:** Hardened `scripts/agent-browser/run.mjs` for CI daemon flakiness by introducing per-flow session IDs, command-level timeouts, tighter command retry loops, and stronger flow-level retry cleanup. Impact: `smoke-agent-browser` no longer spends ~40 minutes stuck in a single ChatShell daemon error cycle before failing and now recovers faster from transient daemon contention.
