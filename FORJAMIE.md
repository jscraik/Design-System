...
- **2026-03-05:** Re-enabled the `risk-policy-gate` job in `.github/workflows/pr-pipeline.yml` (removed unconditional `if: ${{ false }}` skip), corrected its docs baseline step to `node scripts/check-doc-links.mjs`, and added explicit Ralph runtime bootstrapping (`pipx` + pinned `ralph-gold` git spec) in `risk-policy-gate`, `review-gate`, `evidence-verify`, and `remediation-decision` jobs. Impact: PR policy and quality gates are enforceable rather than globally bypassed, and harness stages no longer fail on missing `vale`/`ralph` binaries before policy evaluation.
