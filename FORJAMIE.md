...
- **2026-03-05:** Re-enabled the `risk-policy-gate` job in `.github/workflows/pr-pipeline.yml` (removed unconditional `if: ${{ false }}` skip), corrected its docs baseline step to `node scripts/check-doc-links.mjs`, and switched policy-stage commands to a pinned HTTPS GitHub checkout of `coding-harness` (`pnpm dlx git+https://github.com/jscraik/coding-harness.git#968b60aa030bc16174799082fd2d2ab000a3941b` for `preflight-gate`, `review-gate`, `evidence-verify`, `remediate run`) so CI no longer depends on private npm registry access or SSH key auth on hosted runners. The same pass tightened the dependency-chain `ralph doctor` preflight to fail on unexpected misses while allowlisting absent local agent CLIs (`codex`, `claude`, `copilot`) on GitHub runners. Impact: PR policy and quality gates are enforceable rather than globally bypassed, harness stages run with reproducible public CLI entrypoints in CI, and broken Ralph environments stop the pipeline without blocking hosted runners on optional local-only CLIs.
