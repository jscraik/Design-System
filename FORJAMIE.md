...
- **Harness pre-push runtime repair**: the root workspace now declares local `@brainwav/coding-harness@0.13.1` so `scripts/harness-cli.sh` does not depend on a global install. The wrapper now resolves project mise shims before Homebrew tools, the repo runtime pin/contract use `uv 0.11.3`, and the Semgrep changed-file gate now rebuilds its pinned cache when a broken launcher makes the version probe exit nonzero.
