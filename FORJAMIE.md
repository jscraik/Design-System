...
- **Storybook Playwright server context fix**: `platforms/web/apps/storybook/playwright.visual.config.ts` now launches Storybook with `pnpm exec storybook ...` from the Storybook app directory (`cwd: __dirname`) so visual tests run in the app project context instead of relying on workspace-root command resolution.
