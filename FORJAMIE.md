...
- **2026-03-04:** Hardened `.github/workflows/release-guidance.yml` by scoping install steps to `@brainwav/design-system-guidance` (`pnpm install --filter @brainwav/design-system-guidance...`). This avoids unrelated root/workspace dependency fetch failures during release runs (notably non-published/private dev tooling at repo root). Impact: guidance release workflow can validate/publish independently and reliably.
