...
- **2026-03-04:** Hardened `scripts/agent-browser/run.mjs` for CI daemon flakiness by introducing per-flow session IDs, command-level timeouts, tighter command retry loops, stronger flow-level retry cleanup, and CI read-only mode for ChatShell (validate refs/screenshots but skip flaky send interaction). Impact: `smoke-agent-browser` is no longer blocked by repeated ChatShell click daemon stalls while still validating page structure.
