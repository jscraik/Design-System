...
- **Storybook macOS exemplar tolerance**: the Storybook visual gate now allows up to 1% pixel-ratio drift, scoped only to Storybook screenshots. This matches the measured GitHub macOS Chromium renderer variance after the JSC-76 composition migration while keeping the web visual gate on its stricter threshold.
