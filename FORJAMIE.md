...
- **2026-03-04:** Switched guidance package publish in `.github/workflows/release-guidance.yml` to npm Trusted Publisher (OIDC): removed token-based auth checks, added `id-token: write` permission for publish job, and changed publish command to `npm publish --provenance --access restricted`. Also updated `docs/RELEASE.md` setup guidance accordingly. Impact: GitHub Actions release no longer depends on `BRAINWAV_NPM_TOKEN` for CI publishing and now emits npm provenance attestations.
