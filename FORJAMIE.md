...
- **2026-03-05:** Re-enabled the `risk-policy-gate` job in `.github/workflows/pr-pipeline.yml` (removed unconditional `if: ${{ false }}` skip), corrected its docs baseline step to `node scripts/check-doc-links.mjs`, switched policy-stage commands to `pnpm dlx @brainwav/coding-harness@0.6.0` (`preflight-gate`, `review-gate`, `evidence-verify`, `remediate run`) so the pipeline uses supported harness CLI commands instead of unsupported Ralph subcommands, and tightened the dependency-chain `ralph doctor` preflight to fail on unexpected misses while allowlisting absent local agent CLIs (`codex`, `claude`, `copilot`) on GitHub runners. Impact: PR policy and quality gates are enforceable rather than globally bypassed, harness stages run with valid CLI entrypoints in CI, and broken Ralph environments stop the pipeline without blocking hosted runners on optional local-only CLIs.
