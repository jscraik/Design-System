...
- **2026-02-11:** PR #65 maintenance pass for CI/review hygiene: updated the PR branch from `main` (`gh pr update-branch`) to pull in latest lockfile + chat/a11y fixes, resolved outdated Copilot review threads, corrected workspace version drift by setting `packages/runtime` and `packages/tokens` back to `0.0.1`, switched root `lint`/`format` scripts to `pnpm dlx @biomejs/biome@2.3.13` and `ds:matrix:*` scripts to `pnpm dlx tsx@4.21.0` so CI no longer depends on workspace-hoisted binaries, and replaced the Radix toggle in `packages/effects/src/components/toggle/liquid-toggle.tsx` with a semantic button to satisfy `radix-fallback-only` policy checks. Impact: removes stale review noise and fixes the observed CI blockers (version mismatch + missing CLI binaries + policy violation).
