...
const CODEX_CO_AUTHOR_REGEX = /^Co-authored-by:\s*Codex <noreply@openai\.com>\s*$/im;
...
function main() {
  ...
  if (hasAdditionalContent && lines[firstLineIndex + 1]?.trim() !== "") {
    errors.push("Add a blank line between the subject and the rest of the commit message");
  }
  ...
  if (isAgentBranch && !CODEX_CO_AUTHOR_REGEX.test(commitMsg)) {
    errors.push("Agent branches must include: Co-authored-by: Codex <noreply@openai.com>");
  }
