...
const kebabChar = fc.constantFrom(..."abcdefghijklmnopqrstuvwxyz0123456789".split(""));
        ...
        fc.record({
          templates: fc.array(
            fc.record({
              ...
              tags: fc
                .array(routeToken, { minLength: 0, maxLength: 10 })
                .map((tags) => [...new Set(tags)]), // Ensure uniqueness
              status: fc.constantFrom("alpha", "beta", "stable"),
              route: fc
                .array(routeToken, { minLength: 1, maxLength: 4 })
                .map((parts) => `/${parts.join("/")}`),
