...
function buildMarkdown(rows: MatrixRow[]): string {
  ...
  for (const row of rows) {
    lines.push(
      ...
      )} | ${formatCell(row.a11y_contract_ref)} | ${row.status} | ${formatBool(
        row.web_used,
      )} | ${formatBool(row.tauri_used)} | ${formatBool(row.widget_used)} |`,
    );
  }
...
async function main(): Promise<void> {
  ...
  const surfaceUsage = await readSurfaceUsage();

  const rows: MatrixRow[] = [];

  for (const appExport of appsSdkExports) {
    const usage = resolveSurfaceUsage(appExport.local, surfaceUsage);
    rows.push({
