...
- [x] (2026-01-09 00:00Z) Aligned web app imports/deps/config to @design-studio/ui + @design-studio/runtime.
...
- [x] (2026-01-09 00:00Z) Updated web app imports/config to `@design-studio/ui` following rebrand.
- [x] (2026-01-09 00:00Z) Added/updated tests for registry determinism, routing, search, and a11y.
- [x] (2026-01-09 21:20Z) Audited template components against `@design-studio/ui` and documented gaps.
...
- Observation: Rebrand confirmed; the workspace UI package is `@design-studio/ui`, so any lingering `@design-studio/ui` references must be updated.
  Evidence: `packages/ui/package.json` declares `name: "@design-studio/ui"`.
- Observation: Source templates rely on several local UI primitives (markdown, tag input, shimmer text, etc.) that are not yet exported from `@design-studio/ui`.
...
- Observation: Date range selection already exists in `@design-studio/ui` via the DatePicker module.
  Evidence: `packages/ui/src/components/ui/forms/DatePicker/DatePicker.tsx` exports `DateRangePicker`.

## Decision Log

- Decision: Align the web app to the rebranded `@design-studio/ui` workspace package.
  Rationale: The UI package is now `@design-studio/ui`; reusing it avoids parallel UI foundations and keeps dependency names consistent with the rebrand.
...
- Decision: Add minimal educational/components templates directly in the web app to satisfy category coverage while full source-app porting remains pending.
  ...
  Date/Author: 2026-01-09 / Codex
- Decision: Implement thin wrappers in `@design-studio/ui` for missing template primitives using existing base/overlay components where possible.
...
The web platform app lives at `platforms/web/apps/web`. It currently renders a widget gallery at `/` with a minimal router and a `TemplateWidgetPage` for widget previews. The repo already contains an Apps SDK UI component library at `packages/ui` (published as `@design-studio/ui`) and tokens at `packages/tokens` (published as `@design-studio/tokens`). The source template app is stored under `_temp/ChatGPT UI Templates` and contains a template registry and educational templates not yet integrated into the web platform.
...
First, align package names and imports in the web app to match workspace packages (`@design-studio/ui`, `@design-studio/tokens`, `@design-studio/runtime`). Update `vite.config.ts` and CSS imports accordingly.
...
Next, reconcile template primitives by adding missing wrappers to `packages/ui/src/components/ui` and exporting them from the appropriate `@design-studio/ui` subpaths. These wrappers should either forward to existing Apps SDK UI components (preferred) or encapsulate fallback Radix components where `@design-studio/ui` already maintains fallbacks. The goal is to eliminate direct usage of `_temp_import/components/ui/*` in the template ports.

## Concrete Steps

Run these commands from the repo root unless noted:

- `rg -n "@design-studio/ui|@design-studio/tokens" platforms/web/apps/web`
...
- Audit missing template primitives and implement wrappers:
  ...
  - Ensure wrappers consume tokens and classes from `@design-studio/tokens` and existing theme utilities.
...
- Use `@design-studio/ui` for all components in the template browser and template wrappers.
- Use `@design-studio/runtime` for host providers.
