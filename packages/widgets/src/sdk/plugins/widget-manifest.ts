 ...
 */
function generateTypeScriptManifestContent(manifest: WidgetManifest): string {
...
/**
 * Serialize the manifest into an ESM module for Node MCP runtime imports.
 * @param manifest - The manifest to serialize.
 * @returns The JavaScript module contents.
 */
function generateJavaScriptManifestContent(manifest: WidgetManifest): string {
  return `// This file is auto-generated by the widget-manifest Vite plugin
/**
 * Manifest entries keyed by widget name, with build hash metadata.
 */
export const widgetManifest = ${JSON.stringify(manifest, null, 2)};
`;
}
...
export function widgetManifest(): Plugin {
  ...
  const manifestJsFilePath = resolve(process.cwd(), "src/sdk/generated/widget-manifest.js");
  ...
  return {
    ...
    async configResolved(resolvedConfig) {
      ...
      if (viteConfig.command === "serve") {
        ...
        writeFileSync(manifestFilePath, generateTypeScriptManifestContent(devManifest));
        logger?.info(
    ...
    writeBundle: {
      ...
      async handler(options) {
        ...
        writeFileSync(manifestFilePath, generateTypeScriptManifestContent(buildManifest));
        writeFileSync(manifestJsFilePath, generateJavaScriptManifestContent(buildManifest));
