...
function App() {
  ...
  return (
    <div className="antialiased w-full text-foreground dark:text-foreground px-6 py-5 border border-border dark:border-border rounded-2xl bg-background dark:bg-background">
      <h1 className="sr-only">Pizzaz Markdown</h1>
      <div className="sr-only" aria-live="polite">
        Markdown updated.
      </div>
      <article className="space-y-4 text-sm leading-6 text-foreground dark:text-foreground">
        <ReactMarkdown
          allowedElements={[
            "a",
            "blockquote",
          ...
          components={{
            a: ({ href, children, ...props }) => (
              <a
                {...props}
                href={href}
                className="text-foreground dark:text-foreground underline underline-offset-4 decoration-accent-blue dark:decoration-accent-blue font-medium"
                target="_blank"
            ...
            blockquote: ({ children }) => (
              <blockquote className="border-l-2 border-border dark:border-border pl-4 text-text-secondary dark:text-text-secondary">
                {children}
              </blockquote>
            ),
            code: ({ inline, children }) =>
              inline ? (
                <code className="rounded bg-secondary dark:bg-secondary px-1 py-0.5 text-[0.9em] text-foreground dark:text-foreground">
                  {children}
                </code>
              ) : (
                <code className="block rounded-lg bg-secondary dark:bg-secondary p-3 text-xs leading-5 text-foreground dark:text-foreground">
                  {children}
                </code>
              ),
