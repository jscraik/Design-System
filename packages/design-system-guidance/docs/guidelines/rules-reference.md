...
- `no-empty-accessible-label`: source file contains an empty `aria-label`, `aria-labelledby`, or `title` attribute.
- `no-aria-hidden-focusable`: source file hides a focusable element from assistive technology with `aria-hidden=true`.
- `no-long-motion-duration`: source file contains long Tailwind duration utilities, long inline motion durations, or infinite CSS animation loops.
