...
function renderList(items: string[], emptyText = "None"): string[] {
  return items.length > 0 ? items.map((item) => `- ${item}`) : [`- ${emptyText}`];
}

function renderPrepareStatus(payload: PreparePayload): string {
  return payload.safeForAutomaticImplementation ? "SAFE_TO_IMPLEMENT" : "STOP";
}
...
export function renderPrepareBrief(payload: PreparePayload): string {
  ...
  const lines = [
    ...
    `Status: ${renderPrepareStatus(payload)}`,
