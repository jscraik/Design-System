...
describe("RemoteSkillClient", () => {
    ...
    it("fails on checksum mismatch", async () => {
        ...
        await expect(client.download("demo", { expectedChecksum: "deadbeef" })).rejects.toThrow(/checksum/i);
    });
});
...
const cryptoChecksum = (data) => createHash("sha256").update(data).digest("hex");
