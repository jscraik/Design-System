...
function TabsList({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.List>) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        "bg-secondary text-text-secondary inline-flex h-9 w-fit items-center justify-center rounded-xl p-1 flex",
        className,
      )}
      {...props}
    />
...
function TabsTrigger({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.Trigger>) {
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "data-[state=active]:bg-card data-[state=active]:text-foreground focus-visible:border-interactive focus-visible:ring-2 focus-visible:ring-ring text-text-secondary inline-flex h-[calc(100%-0.0625rem)] flex-1 items-center justify-center gap-1.5 rounded-xl border border-transparent px-2 py-1 text-body-small font-medium whitespace-nowrap transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
