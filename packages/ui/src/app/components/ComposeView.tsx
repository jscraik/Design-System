...
const getTaskSectionConfig = (promptEnhancement: 'rewrite' | 'augment' | 'preserve') => {
    ...
    <div className="bg-[#212121] border border-[#4d4d4d] rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-[#2a2a2a]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[15px] leading-6 text-[#ececec] font-medium">Instructions</span>
          <button className="p-1 hover:bg-white/10 rounded transition-colors">
            <svg
              className="size-4 text-[#ececec]/70"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
...
function ModelSelector({
  ...
  return (
    <div className="w-[340px]">
      <label className="block text-[13px] leading-5 text-[#ececec]/70 mb-2">Model</label>
      <div className="relative">
        <div className="bg-[#2d2d2d] rounded-lg p-0.5">
          <button
            className="flex items-center justify-between hover:bg-white/10 px-3 py-2 rounded-md transition-colors w-full"
            onClick={onToggle}
          >
...
function TaskDescriptionSection({
  ...
  taskConfig: { label: string; placeholder: string; tooltip: string; buttonText: string };
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between mb-2">
        <label className="text-[13px] leading-5 text-[#ececec]/70">{taskConfig.label}</label>
  ...
  return (
    <div className="flex items-center justify-between pt-5 border-t border-[#4d4d4d]">
      <div className="flex items-center gap-4">
        <div>
          <div className="text-[13px] leading-5 text-[#ececec]">Auto Plan</div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-[12px] leading-[18px] text-[#ececec]/70">Plan mode</span>
          <div className="relative">
            <button
...
function ContentBuilderSection({
  selectedModel,
  onToggleModelPicker,
  taskConfig,
  taskDescription,
    ...
    <div className="bg-[#212121] border border-[#4d4d4d] rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-[#2a2a2a]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[15px] leading-6 text-[#ececec] font-medium">Content Builder</span>
        </div>
        <button className="flex items-center gap-2 px-3 py-1.5 bg-[#2f2f2f] hover:bg-[#3f3f3f] border border-[#4d4d4d] text-[#ececec] rounded-lg transition-colors text-[13px] leading-5">
      ...
      <div className="p-6 space-y-6">
        <div className="space-y-5">
          <ModelSelector selectedModel={selectedModel} onToggle={onToggleModelPicker} />
          ...
          <div className="mb-4">
            ...
            <div className="space-y-1">
              {availableModels.slice(0, 3).map((model) => (
                ...
                >
                  <div>
                    <div className="text-[14px] font-normal leading-[20px] tracking-[-0.3px]">
                      {model.name}
                    </div>
...
function ModeSelectorModal({
  ...
  onHoverMode: (mode: ModeConfig) => void;
  onClose: () => void;
  onOpenProEditConfig: () => void;
}) {
  return (
    <>
      <div className="fixed inset-0 bg-black/40 z-40" onClick={onClose} />
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="px-4 py-3 bg-white/5 border border-white/10 rounded-lg">
                  <div className="text-[12px] font-normal leading-[18px] tracking-[-0.32px] text-white/60">
                    Selected Files ·{' '}
                    <span className="text-white">{previewMode.contextConfig.selectedFiles}</span>
                  </div>
                </div>
                <div className="px-4 py-3 bg-white/5 border border-white/10 rounded-lg">
                  <div className="text-[12px] font-normal leading-[18px] tracking-[-0.32px] text-white/60">
...
function ProEditConfigModal({
  proEditMode,
  onChangeProEditMode,
  selectedAgent,
  onChangeSelectedAgent,
  selectedModelConfig,
  onChangeSelectedModelConfig,
  onClose,
}: {
  proEditMode: 'agent' | 'model';
  onChangeProEditMode: (mode: 'agent' | 'model') => void;
  selectedAgent: string;
  onChangeSelectedAgent: (value: string) => void;
  selectedModelConfig: string;
  onChangeSelectedModelConfig: (value: string) => void;
  onClose: () => void;
}) {
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[60] w-[720px] bg-[#171717] border border-white/10 rounded-[12px] shadow-2xl p-8">
        ...
        <div className="inline-flex items-center gap-0 bg-[#2a2a2a] rounded-lg p-1 mb-6">
          <button
            onClick={() => onChangeProEditMode('agent')}
            className={`px-4 py-1.5 rounded-md transition-all text-[14px] font-normal leading-[20px] tracking-[-0.3px] ${
              proEditMode === 'agent' ? 'bg-[#2f7a4f] text-white' : 'text-[#ababab] hover:text-white'
            }`}
          >
            Agent
          </button>
          <button
            onClick={() => onChangeProEditMode('model')}
            className={`px-4 py-1.5 rounded-md transition-all text-[14px] font-normal leading-[20px] tracking-[-0.3px] ${
              proEditMode === 'model' ? 'bg-[#2f7a4f] text-white' : 'text-[#ababab] hover:text-white'
            }`}
          >
            Model
          </button>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => onChangeSelectedModelConfig(e.target.value)}
                    className="w-full bg-[#2a2a2a] border border-white/10 rounded-lg px-4 py-2.5 text-[14px] text-white font-normal leading-[20px] tracking-[-0.3px] appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-white/20"
                  >
...
export function ComposeView() {
  ...
  const [taskDescription, setTaskDescription] = useState(
    'Review Codex CLI codebase and identify methods for decomposing work into a graph'
  );
    ...
    <div className="flex-1 flex flex-col bg-[#0D0D0D]">
      <div className="flex-1 overflow-y-auto">
        <div className="w-full px-8 py-8 space-y-8">
          <InstructionsPanel instructions={instructions} onChange={setInstructions} />
          ...
          <div className="min-h-[200px]">
            {activeTab === 'builder' && (
              <ContentBuilderSection
                ...
                autoPlan={autoPlan}
                onToggleAutoPlan={() => setAutoPlan(!autoPlan)}
                previewMode={previewMode}
                onOpenModeSelector={handleModalOpen}
              />
            )}
          </div>
        </div>
      ...
      {showModeSelector && (
        <ModeSelectorModal
          ...
          onHoverMode={handleModeHover}
          onClose={() => setShowModeSelector(false)}
          onOpenProEditConfig={() => setShowProEditConfig(true)}
        />
      )}
      ...
      {showProEditConfig && (
        <ProEditConfigModal
          ...
          onChangeProEditMode={setProEditMode}
          selectedAgent={selectedAgent}
          onChangeSelectedAgent={setSelectedAgent}
          selectedModelConfig={selectedModelConfig}
          onChangeSelectedModelConfig={setSelectedModelConfig}
