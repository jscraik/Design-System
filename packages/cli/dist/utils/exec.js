...
function buildRunResultData(result) {
    return {
...
function buildRunResultError(result) {
    if (result.exitCode === 0)
        return [];
    return [
        toJsonError(new CliError("Command failed", {
...
export function emitResult(opts, result, summary) {
    ...
    if (opts.plain) {
        ...
        outputPlainRecord({
            ...
            stdout: includeOutput ? data.stdout : undefined,
            stderr: includeOutput ? data.stderr : undefined,
            error_code: result.exitCode === 0 ? undefined : ERROR_CODES.exec,
            error_message: result.exitCode === 0 ? undefined : "Command failed",
        });
    }
}
export async function handleRun(opts, args, summary, target) {
